...
async def review_readings(data: SupervisorReviewInput, ctx: dict = Depends(get_station_context)):
    ...
    # Note: approving ER readings does NOT auto-advance the container shift. Attendants
    # who only use Enter Readings (no financial handover) must be included in a shift
    # that is closed separately. The unified review queue surfaces them alongside
    # handover submissions but the shift completion gate lives in attendant_handover.py.
